...
public class GasSucker : KMonoBehaviour, ISim1000ms, ISaveLoadable, ISingleSliderControl, ISliderControl
{
	...
	[MyCmpReq]
	private LogicOperationalController lc;
	...
	private void Emit(float dt)
	{
		...
		Debug.Log("logic:" + lc.isActiveAndEnabled);
		if (myCell <= 0)
        {
			return;
        }
