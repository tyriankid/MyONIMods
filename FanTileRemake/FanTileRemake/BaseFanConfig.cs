        ...
        public static void DoPostConfigureComplete(GameObject go, float suckRate, ConduitType conduitType, float overPressureThreshold)
        {
            ...
            switch (conduitType)
            {
                case ConduitType.Gas:
                    //新增可筛选元素
                    go.AddOrGet<FilterbleElementGas>().elementState = Filterable.ElementState.Gas;
                    elementConsumer1.configuration = ElementConsumer.Configuration.AllGas;
                    break;
                case ConduitType.Liquid:
                    //新增可筛选元素
                    go.AddOrGet<FilterbleElementGas>().elementState = Filterable.ElementState.Liquid;
            ...
            storage.capacityKg = 50000;//2 * suckRate;
